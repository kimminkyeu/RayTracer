...
int do_ray_tracing_and_return_color(t_device *device, t_image *img, int x, int y)
{
	...
	trace_result = clamp3(trace_result, \
                                vec3_1f(0.0f), vec3_1f(255.0f));
...
t_vec3	do_recur_super_sampling(t_image *img, t_vec3 pixel_pos_world, const float sub_dx, const int super_sampling_recursive_level)
{
	...
	while (j < 2)
	{
		...
		while (i < 2)
		{
			t_vec3 sub_pos = vec3_3f(pixel_pos_world.x + ((float) i * sub_dx), \

									 pixel_pos_world.y + ((float) j * sub_dx), \

									 pixel_pos_world.z);
			pixel_color = add3(pixel_color,
							   super_sampling_anti_aliasing(img,
															sub_pos,
															sub_dx,
															super_sampling_recursive_level -
															1));
			i++;
		}
		j++;
	...
	while (j < 2)
	{
		...
		while (i < 2)
		{
			t_vec3 sub_pos = vec3_3f(pixel_pos_world.x + ((float) i * sub_dx), \
									 ...
									 pixel_pos_world.z);
			t_vec3 ray_dir = normal3(sub3(sub_pos,
										  img->device_ptr->camera->pos));

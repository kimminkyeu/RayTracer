...
{
	const float	_diff = max_float(dot3(hit.normal, hit_point_to_light), 0.0f) * light->brightness_ratio;
...
t_vec3	calcalate_specular(const t_ray *ray, t_hit hit, t_light *light, t_vec3 hit_point_to_light)
{
	const t_vec3	spec_reflection_dir = sub3(mult3_scalar(
			mult3_scalar(hit.normal,
						 dot3(hit_point_to_light,
							  hit.normal)),
			2.0f), hit_point_to_light);
	const float		_spec = powf(max_float(dot3(vec3_reverse(ray->direction),
												   spec_reflection_dir), 0.0f), hit.obj->material.alpha * light->brightness_ratio);
	const t_vec3	specular_final = mult3_scalar(mult3_scalar(light->color,
															   _spec),
												  hit.obj->material.ks);
...
{
	const t_vec3	hit_point_to_light = normal3(sub3(light->pos,
													  hit.point));

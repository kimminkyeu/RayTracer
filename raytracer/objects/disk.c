...
{
	if (abs_float(gl_vec3_dot(ray->direction, disk->orientation)) < 0.001f)
		return (true);
	else
		return (false);
}
...
t_hit	disk_intersect_ray_collision(const t_ray *ray, t_disk *disk)
{
	...
	if (is_backface(ray, disk) || is_ray_horizontal_to_surface(ray, disk))
		return (hit);
	t = (gl_vec3_dot(disk->center, disk->orientation) \
		- gl_vec3_dot(ray->origin, disk->orientation)) \
		/ gl_vec3_dot(ray->direction, disk->orientation);
	if (t < 0.0f)
		return (hit);
	hit.distance = t;
	hit.point = gl_vec3_add_vector(ray->origin, \
		gl_vec3_multiply_scalar(ray->direction, t));
	hit.normal = disk->orientation;
	hit_to_center = abs_float(gl_vec3_get_magnitude(\
				gl_vec3_subtract_vector(hit.point, disk->center)));
